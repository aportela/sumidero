var sumideroUpdatePost = (function () {
    "use strict";
            ...
            <form v-on:submit.prevent="update();">
                ...
                <div class="field is-horizontal">
                    ...
                    <div class="field-body">
                        <div class="field">
                            <div class="control">
                                <button type="submit" :disabled="! title" class="button is-primary" v-bind:class="loading ? 'loading' : ''">Update</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    var module = Vue.component('sumidero-update-post', {
        template: template(),
        data: function () {
            return ({
                loading: false,
        ...
        methods: {
            getPost: function (permaLink) {
                ...
                sumideroAPI.getPost(permaLink, function (response) {
                    if (response.ok) {
                        ...
                        self.tagNames = response.body.post.tags;
                        self.loading = false;
                    } else {
                        self.errors = true;
            ...
            update: function () {
                ...
                self.loading = true;
                self.apiError = null;
                sumideroAPI.updatePost(this.id, this.externalUrl, this.title, this.body, this.sub, this.tags, this.thumbnail, function (response) {
                    if (response.ok) {
                    } else {
                        self.apiError = response;
                    }
